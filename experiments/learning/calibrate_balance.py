"""Calibrate E/I balance: inhibitory gain and/or global weight scale
that kills ignition while preserving sugar->MN9."""
import json, sys
...
for i_gain, w_scale in [(2.0, 1.0), (3.0, 1.0), (5.0, 1.0), (1.0, 0.5), (1.0, 0.7), (2.0, 0.7)]:
    ...
    print(f'i_gain={i_gain:3.1f} w_scale={w_scale:3.1f}  kick: active={int((e_kick>0).sum()):6d} '
          f'KCact={int((e_kick[kc_idx]>=3).sum()):5d}  sustained={int((e_sil>0).sum()):6d}  '
          f'sugar: MN9={float(e_sug[mn9_idx].mean()):6.1f}Hz feeding={int((e_sug>0).sum()):5d}', flush=True)
